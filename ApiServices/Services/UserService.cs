...
public class UserService(AppDbContext dbContext, RoleService roleService)
{
	...
	public async Task<ServiceFlag<User?>> FindBy(
		Guid? id = default,
		string? name = default,
	...
	{
		var (status, user, _) = await FindBy(id: details.UserId);
	...
	public async Task<ServiceFlag<User?>> Delete(Guid id)
	{
		...
		if (user is not { Active: true }) return new ServiceFlag<User?>(HttpStatusCode.NotFound);
		user.Active = false;
