...
public class AuthorizeRoleAttribute : TypeFilterAttribute {
	public AuthorizeRoleAttribute(params UserRole.Type[] requiredRoles) : base(typeof(AuthorizeFilter)) => Arguments = [
		requiredRoles
	];
}

public class AuthorizeFilter(AuthService authService,
	UserRole.Type[]? requiredRoles = default) : IAsyncActionFilter {
	public async Task OnActionExecutionAsync(ActionExecutingContext context,
	ActionExecutionDelegate next) {
		if (requiredRoles == null) { return; }
		
		try {
			var (status, _, _) = await authService.Authorize(context.HttpContext,
				requiredRoles);
